...
//  SwiftDemo
//
//  Created by MA806P on 2018/1/13.
...
class PlayingCardView: UIView {
    ...
    private lazy var uperLeftCornerLabel = createCornerLabel()
    ...
    override func layoutSubviews() {
        ...
        configureCornerLabel(uperLeftCornerLabel)
        uperLeftCornerLabel.frame.origin = bounds.origin.offsetBy(dx: cornerOffset, dy: cornerOffset)
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            ...
            if rank < 11 && rank > 0 {
                if let cardBackImage = UIImage(named: "01", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
                    cardBackImage.draw(in: bounds.zoom(by: SizeRatio.faceCardImageSizeToBoundsSize))
                }
            } else {
                if let cardBackImage = UIImage(named: "02", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
            ...
            if let cardBackImage = UIImage(named: "03", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
